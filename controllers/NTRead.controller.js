import { probWireTampQuery } from "../utils/DBQueries/index.js";
...
async function probWireTamp(req, res) {
  ...
  try {
    ...
    if(!result){
        return res.status(StatusCodes.NOT_FOUND).json(new ApiErrorResponse(StatusCodes.NOT_FOUND, "Data not Found"))
    };

    return res.status(StatusCodes.OK).json(new ApiSuccessResponse(StatusCodes.OK, "Successfully Fetched the data", encryptData(result)))
