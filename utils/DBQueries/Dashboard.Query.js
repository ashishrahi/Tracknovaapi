...
export const getDashboardQuery = async (CurDate) => {
  ...
  summaryNTDash.IgnitionStatus = ignitionStatusResult
    .map((r) => `${r._id}~${r.count}`)
    .join(", ");

  // Zone vehicle status query
  const zoneDeviceIds = await NTCurrentDay.distinct("devid", {
    TrackTime: { $gt: CurDate },
  });
  const zoneVehicleStatusResult = await ItemMaster.aggregate([
    { $match: { devid: { $in: zoneDeviceIds } } },
    { $group: { _id: "$ZoneName", count: { $sum: 1 } } },
  ]).exec();
  summaryNTDash.ZoneVehicleStatus = zoneVehicleStatusResult
    .map((r) => `${r._id}~${r.count}`)
    .join(", ");
  ...
  const vehicleCounts = await NTCurrentDay.aggregate([
    { $match: { TrackTime: { $gt: CurDate } } },
    {
      $facet: {
        ...
        idle: [{ $match: { acc: true, speedDecimal: 0 } }, { $count: "count" }],
        stop: [
  ...
  const totalVehicleRunningCount = await NTCurrentDay.distinct("devid", {
    TrackTime: { $gt: CurDate },
  });
  summaryNTDash.TotalVehicleRunning =
    totalVehicleRunningCount.length.toString();

  // GPS status count
  const gpsStatusCount = await ItemMaster.countDocuments({
    ItemFlag: "V",
    devid: { $ne: null },
  });
