...
export const AddUpdateRosterPlanQuery = async (model) => {
  try {
    const { RosterPlan, RosterPlanDetail } = await getTenantDBModels();

    if (model.rosterID === 0) {
      // Check if RosterNo already exists
      const existingRecord = await RosterPlan.findOne({
      ...
      const lastRecord = await RosterPlan.findOne()
        .sort({ RosterID: -1 })
        .lean();
      ...
      const existingEntity = await RosterPlan.findOne({
        RosterID: model.rosterID,
      });
      if (!existingEntity) {
        return {
      ...
      if (
        Array.isArray(model.RosterPlanDetails) &&
        model.RosterPlanDetails.length > 0
      ) {
        model.RosterPlanDetails.forEach(
          (detail) => (detail.RosterID = model.RosterID)
        );
...
export const GetRosterPlanQuery = async (model) => {
  try {
    const { RosterPlan } = await getTenantDBModels();
    ...
    return {
      ...
      error:
        rowCount === 0
          ? { ErrorMessage: "No records found", StatusCode: 404 }
          : null,
    };
  } catch (error) {
    return {
...
export const DeleteRosterPlanQuery = async (model) => {
  try {
    const { RosterPlanDetail, RosterPlan } = await getTenantDBModels();

    const cam = await RosterPlanDetail.findOne({
      rosterID: model.rosterID,
    }).lean();
    if (cam) {
      return {
