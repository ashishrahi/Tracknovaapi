...
export const TripStartEndQuery = async (model) => {
        ...
        if (model.isStart) {
          ...
          const item = await ItemMaster.findOne({ VehicleNo: model.VehicleNo.trim() });
          if (!item) {
           return{
             isSuccess: false,
          ...
          const routemap = await RouteVehicleMapping.findOne({ ItemId: item.ItemMasterId });
          if (!routemap) {
           return{
             isSuccess: false,
             statusCode: StatusCodes.NOT_FOUND,
          ...
          let tripId = tripIds.length > 0 ? Math.max(...tripIds.map(trip => trip.TripId)) + 1 : 1;
